...
interface SessionSideListItemProps {
  ...
  /** 제목 수정 (활성 항목에만 제공 — 현재 상담 기록 제목 편집) */
...
export const SessionSideListItem: React.FC<SessionSideListItemProps> = ({
  ...
  // Escape로 닫을 때 blur가 저장을 트리거하지 않도록 하는 플래그
  const escapedRef = React.useRef(false);

  const canEdit = isActive && !!onTitleUpdate;

  // 외부 title 변경(저장 후 재조회) 시 편집 중이 아니면 동기화
  React.useEffect(() => {
    if (!isEditing) setEditedTitle(title);
  }, [title, isEditing]);
  ...
  const handleSave = async () => {
    ...
    if (!onTitleUpdate || !next || next === title) {
      setIsEditing(false);
  ...
  // 저장 경로를 blur 하나로 통일: Enter=blur(→저장), Escape=플래그+닫기(저장 스킵)
  const handleBlur = () => {
  ...
  const advancedBadge = isAdvancedTranscript && (
    <svg
      width="14"
      height="14"
      viewBox="0 0 14 14"
  ...
  // 활성(현재) 세션: 네비게이션 대신 인라인 제목 편집 가능한 카드
  if (isActive) {
    return (
      <div className="w-full rounded-lg bg-surface-contrast px-3 py-2 text-left">
        <Title
          as="h4"
          className="typo-sm flex items-center justify-between gap-1 font-emphasize text-fg"
        >
          {isEditing ? (
            <input
              ref={inputRef}
              ...
              onBlur={handleBlur}
              disabled={isSaving}
              className="min-w-0 flex-1 rounded border border-green-80 bg-white px-1 py-0.5 text-fg focus:outline-none"
            />
          ) : (
            <span className="flex min-w-0 items-center gap-1">
              ...
              {canEdit && (
                <button
                  ...
                  aria-label="제목 수정"
                  className="flex-shrink-0 text-grey-60 lg:hover:text-grey-100"
                >
                  <TitleEdit size={13} />
                </button>
              )}
  ...
  return (
    <button
      ...
      className="w-full rounded-lg px-3 py-2 text-left transition-colors lg:hover:bg-surface-contrast"
    >
      <Title
        as="h4"
