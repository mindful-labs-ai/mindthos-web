...
export function AddMemberCard({ onClick, disabled }: AddCardProps) {
  return (
    <button
      ...
      disabled={disabled}
      className={cn(
        'flex h-[276px] items-center justify-center rounded-lg border border-dashed transition-colors',
        disabled
...
export function AddRelationCard({ onClick, disabled }: AddCardProps) {
  return (
    <button
      ...
      disabled={disabled}
      className={cn(
        'flex h-[169px] w-full items-center justify-center rounded-lg border border-dashed transition-colors',
        disabled
          ? 'cursor-not-allowed border-border opacity-50'
          : 'border-fg-muted lg:hover:border-fg'
      )}
