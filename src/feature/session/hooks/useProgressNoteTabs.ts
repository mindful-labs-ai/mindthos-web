...
export interface UseProgressNoteTabsReturn {
  ...
  // API 요청 중인 탭 관리
  requestingTabs: Record<string, { templateId: number; progressNoteId: string | null }>;
  setRequestingTabs: React.Dispatch<
    React.SetStateAction<Record<string, { templateId: number; progressNoteId: string | null }>>
  >;
...
export function useProgressNoteTabs({
  ...
  // 폴링 훅
  const { processingNoteIds, hasProcessingNotes } = useSessionProgressNotesPolling({
    sessionId,
    isDummySession,
    ...
    onNoteError: (note) => {
      ...
      toast({
        title: '상담노트 작성 실패',
        description: note.error_message || '상담노트 작성 중 문제가 발생했습니다.',
        duration: 5000,
      });
    },
  });
  ...
  React.useEffect(() => {
    ...
    if (validNotes.length > 0) {
      ...
      const latestByTemplate = validNotes.reduce(
        (acc, note) => {
          const templateId = note.template_id ?? 'null';
          const existing = acc.get(templateId);
          if (
