 ...
 * 서명하면 모든 동의에 적용된다. 구성: "{상담사} 상담사" / 날짜 / "내담자 {이름}" / 서명란(서명 이미지
 * 또는 빈 서명선). 내담자 이름은 실제 발송/제출에선 내담자명, 양식(미리보기)에선 "OOO"로 표시한다.
...
export function DocumentSignatureFooter({
  ...
  return (
    <div className="flex flex-col items-end gap-3 text-sm text-grey-100">
      {counselorName && <p className="font-bold">{counselorName} 상담사</p>}
      {dateLabel && <p>{dateLabel}</p>}
      <div className="flex items-end gap-3">
        <span className="text-grey-70">내담자</span>
