...
export function SharedDocumentIntro({
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-[#F4F5FA]">
      <SharedHeader title={title} onBack={onBack} />

      {/* 히어로 — 상담사가 요청한 문서 안내 + 흐린 문서 아이콘 */}
      <div className="relative px-6 pb-9 pt-8">
        <h2 className="max-w-[260px] text-2xl font-bold leading-[150%] text-[#3C3C3C]">
        ...
        <FileText
          aria-hidden
          strokeWidth={1.5}
          className="pointer-events-none absolute right-6 top-[72px] h-[86px] w-[86px] text-[#44CE4B] opacity-20"
        />
      </div>

      {/* 흰 패널 — 안내 문구 + 동의 + CTA */}
      <div className="flex flex-1 flex-col bg-white px-6 pb-7 pt-8">
        <div className="flex flex-col gap-5">
          <p className="whitespace-pre-line text-base font-medium leading-[150%] text-[#3C3C3C]">
        ...
        <div className="flex items-center gap-3 py-2">
          ...
          >
            <span className="text-base font-medium text-[#747479]">
              [필수] 민감정보수집 및 이용 동의
            </span>
            <ChevronRight
              size={24}
              strokeWidth={2}
              className="flex-shrink-0 text-[#747479]"
            />
          </button>
        </div>
