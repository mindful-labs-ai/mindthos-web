# feature/parser-refactor 리뷰 가이드 (2026-07-12, 최종 갱신 07-13)

develop 기반 로컬 브랜치, 총 **23커밋** (미푸시). 파서 리팩터링 → 디자인시스템
역산·중앙화 → 코드리뷰·후속 조치까지 4단계로 진행. 커밋 단위가 곧 리뷰 단위다.

- 상세 디자인시스템 분석: [DESIGN_SYSTEM_AUDIT_20260712.md](./DESIGN_SYSTEM_AUDIT_20260712.md)
- 검증: `pnpm typecheck` ✓ · `pnpm lint`(에러 0) ✓ · 단위 테스트 **429건** ✓ · `pnpm build` ✓
  · 브라우저 전/후 비교(하단 §4) ✓ · 독립 코드리뷰 **APPROVE** (하단 §6)
...
4. **축어록 비언어 칩** (세션 상세): 뷰(읽기 모드)는 색 변화 없어야 정상 (토큰 승격만).
   **편집 모드**의 레거시 침묵/겹침 칩만 회색/보라로 변경 (§6, 뷰와 정합)
...
## 6. 4단계 — 코드리뷰와 후속 조치 (07-12~13, 4커밋)

독립 리뷰어 2종(품질·테스트) + 자체 검증으로 브랜치 전체를 리뷰. **판정 APPROVE**
(고위험 8개 영역 전수 통과: deid 캡처그룹 정합, 벌크 치환 비클래스 오염 없음,
radius/shadow 별칭 안전, hooks deps 무변경, 다크 토큰 미러링 등).

| 커밋 | 내용 | 픽셀 영향 |
|---|---|---|
| `88f43c6` | **리뷰 중 발견한 리그레션 수정**: Tailwind v3는 `var()` 색상에 `/알파` 수정자를 지원하지 않아 `bg-green-80/10`이 CSS 미생성 → 캘린더 드래그 하이라이트 투명화. 알파 포함 토큰 `--color-primary-tint`(10%)·`tint-weak`(8%)로 복원 | 리그레션 복원 (원래 모습으로) |
| `845cf28` | 편집기 레거시 S/O 칩 색을 뷰와 정합 (`NV_CHIP_COLORS[tagType]` 직접 사용) | **의도적**: 편집 모드에서 침묵 칩 파랑→회색, 겹침 칩 파랑→보라. 직렬화는 `data-legacy-tag` 속성 기반이라 저장 데이터 무영향 (왕복 테스트로 증명) |
| `ed70399` | 리뷰 우선순위 테스트 5건 보강 (+26건, 총 429건) | 없음 |
| `40734e6` | 본 문서 | — |

**커버리지 개선**: getSpeakerInfo 54.5→95.5% · SegmentContentEditor 0→50.7%(데이터
경로 전부) · useTranscriptCopy 0→83.3% · contentsEditor 브랜치 71.6→82.4% ·
transcriptTags 100%.

**리뷰에서 확인된 잔여 이슈 (모두 브랜치 이전부터 존재, 비회귀)**:

1. **[Major] `var()` 색상 알파 수정자 CSS 미생성 — 13개 파일**: `bg-primary/10`,
   `bg-accent/10`, `bg-orange-100/10` 등이 처음부터 스타일 누락 (Button accent hover,
   Chip primary/accent 톤 배경, 편집기 deid 칩 배경 등). 고치면 "의도한 배경이 처음
   생기는" 시각 변화 → 디자인 확인 후 `primary-tint` 방식(알파 포함 토큰)으로 일괄
   수정 권장.
2. **[Low] 태그 특수문자 왕복 엣지**: 레거시 라벨에 `%`, deid 원본에 `⟫` 입력 시
   오파싱. 이스케이프 정책 결정 필요.
3. **[Low] `extractTextOnly`**: nv 배열 존재 시 레거시 태그 미제거 (실데이터에선
   혼재하지 않아 무해, 방어적 보강 기회).
